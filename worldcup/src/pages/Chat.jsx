...
// שולח פוש דרך Edge Function
async function sendPushNotification(type, data, excludeEmail) {
  try {
    await supabase.functions.invoke('send-push', {
      body: { type, data, exclude_email: excludeEmail },
    });
  } catch (e) {
...
export default function Chat() {
  ...
  const send = async (e) => {
    ...
    const nickname = user.nickname || user.full_name || user.email;
    const message = input.trim();

    await chatApi.send(user.email, nickname, message);

    // שלח פוש לכולם חוץ מהשולח
    await sendPushNotification('chat', { nickname, message }, user.email);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)]">
      <h1 className="text-2xl font-black flex items-center gap-2 mb-4">
        <MessageCircle size={24} className="text-primary" />צ'אט
      </h1>
