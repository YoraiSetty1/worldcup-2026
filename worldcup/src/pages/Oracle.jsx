...
export default function Oracle() {
  ...
  const askOracle = async (match) => {
    ...
    try {
      ...
      const prompt = `אתה אנליסט נתוני ספורט ומומחה הימורי כדורגל עבור אפליקציית טורניר חברים למונדיאל 2026.
מידע קריטי: הטורניר משוחק במגרשים ניטרליים בלבד. המושגים "קבוצת בית" ו"קבוצת חוץ" הם טכניים בלבד (לצורך הצגת הלו"ז) ואין להם שום השפעה על יתרון מקצועי או ביתיות. התעלם לחלוטין מיתרון ביתיות בניתוח שלך.

נתח את המשחק הבא: ${match.home_team_name} נגד ${match.away_team_name}.
בסס את הניתוח אך ורק על יחסי כוחות נטו, איכות סגל, דירוג פיפ"א וכושר נוכחי.

החזר תשובה בפורמט JSON טהור בלבד. השדות:
- home_win_pct (מספר)
- draw_pct (מספר)
- away_win_pct (מספר)
- predicted_score (מחרוזת בפורמט "X-Y", חובה שהתוצאה תתאים מתמטית להסתברויות שניחשת)
- analysis (טקסט קצר בעברית של 2-3 משפטים עם המלצת הימור, ללא אזכור יתרון ביתיות)`;
