...
export default function Oracle() {
  ...
  const askOracle = async (match) => {
    ...
    try {
      ...
      const prompt = `אתה אנליסט נתוני ספורט ומומחה הימורי כדורגל למונדיאל 2026.
מידע קריטי: המגרשים ניטרליים, אין יתרון ביתיות. התעלם ממושגי "בית" או "חוץ".

נתח את המשחק: ${match.home_team_name} נגד ${match.away_team_name}.
בסס את הניתוח על דירוג פיפ"א, כושר נוכחי ואיכות סגל.

החזר JSON טהור:
{
  "home_win_pct": (מספר 0-100),
  "draw_pct": (מספר 0-100),
  "away_win_pct": (מספר 0-100),
  "predicted_score": (מחרוזת "X-Y"),
  "analysis": (ניתוח קצר)
}

חוקי ברזל:
1. סכום האחוזים חייב להיות 100.
2. ה-predicted_score חייב לשקף את פערי הרמות (למשל, אם ניצחון קבוצה א' הוא 70%, התוצאה לא יכולה להיות 1-2).
3. **אסור לחזור על תוצאות!** גוון את התוצאות (0-0, 2-0, 3-1, 1-1, 0-1 וכו'). אני בודק את הגיוון שלך.
4. השתמש בהיגיון טקטי - אם קבוצה הגנתית, התוצאה תהיה נמוכה. אם התקפית, גבוהה.`;
