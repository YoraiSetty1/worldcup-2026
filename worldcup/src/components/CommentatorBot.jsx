...
export default function CommentatorBot() {
  ...
  const handleSend = async () => {
    ...
    try {
      const prompt = `אתה אנליסט וסטטיסטיקאי כדורגל מקצועי ורציני, מומחה למונדיאל וטורנירים בינלאומיים.
      ענה על השאלה הבאה של המשתמש בצורה עניינית, מדויקת ומבוססת עובדות בלבד. ללא ציניות, ללא הומור וללא סלנג.
      ספק נתונים היסטוריים אמיתיים, סטטיסטיקות, וניתוח מקצועי. במידה ומדובר בתחזית או השערה, ציין זאת במפורש.
