...
export default function CommentatorBot() {
  ...
  const handleSend = async () => {
    ...
    try {
      const prompt = `אתה פרשן כדורגל ציני וחד, מומחה למונדיאל 2026, שיושב בתוך אפליקציית הימורים של חברים.
      ענה על השאלה הבאה של המהמר בסלנג ישראלי קליל, מקצועי (תמציא נתונים הגיוניים אם חסר לך מידע לעתיד) אבל עוקצני, ותן תשובה קצרה וקולעת (עד 3 משפטים מקסימום).
